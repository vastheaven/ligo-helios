...
def password_voter_login(request, election):
  ...
  if password_login_form.is_valid():
    try:
      ...
      user = get_user(request)
      if user:
        voter = election.voter_set.get(voter_login_id = user.user_id,
                                     voter_password = "ligohelios")
        request.session['CURRENT_VOTER'] = voter
      else:
       # voter = election.voter_set.get(voter_login_id = "xxx",
       #                              voter_password = "xxx")
        return HttpResponseRedirect("/auth/?return_url="+return_url)
